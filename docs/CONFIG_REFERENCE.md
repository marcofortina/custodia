...
`custodia-client` uses a per-user JSON profile generated by `custodia-client config write --client-id ID --server-url URL`. The standard profile path is `$XDG_CONFIG_HOME/custodia/ID/ID.config.json`, or `$HOME/.config/custodia/ID/ID.config.json` when `XDG_CONFIG_HOME` is not set. `custodia-admin` intentionally has no persistent config file; admin commands should keep `--server-url`, `--cert`, `--key` and `--ca` explicit, or use environment overrides for automation.
