...
`custodia-client` uses a per-user JSON profile generated by `custodia-client config write --client-id ID` after `custodia-client mtls enroll` has stored the server URL. The standard profile path is `$XDG_CONFIG_HOME/custodia/ID/ID.config.json`, or `$HOME/.config/custodia/ID/ID.config.json` when `XDG_CONFIG_HOME` is not set. `custodia-admin` intentionally has no persistent client profile. First-run admin reads such as `status read`, `diagnostics read` and helpers such as `client enrollment create` can derive local transport defaults from `/etc/custodia/custodia-server.yaml`; explicit `--server-url`, `--cert`, `--key` and `--ca` flags still override those defaults.
