...
- Kubernetes bootstrap material evidence includes namespace and Secret creation, Web MFA Secret creation, admin browser certificate package handling, a decision on whether Full signer PKCS#11 delivery uses a custom image or volume mount, and whether the optional non-secret bootstrap Job was enabled.
- Installed Kubernetes release passes [`KUBERNETES_RUNTIME_SMOKE.md`](KUBERNETES_RUNTIME_SMOKE.md) read-only cluster checks for ConfigMap profile/backend wiring, server/signer rollouts, Services, signer ClusterIP-only exposure, optional bootstrap Job completion and Lite PVC presence when applicable.
