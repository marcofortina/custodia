...
For Kubernetes, ordinary online administration should use the Web Console/API rather than `kubectl exec` into application pods. The Web Console covers status, diagnostics, client metadata, one-shot enrollment tokens, future client revocation, client-CRL status, access request views, audit views, audit JSONL export download and audit-chain verification. Bootstrap, Kubernetes Secrets, Helm values, CA/HSM placement and backup plumbing remain deployment runbook tasks outside the Web Console.
